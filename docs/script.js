const algorithms = [
    {
        id: "fraudulent-activity-notifications",
        title: "Fraudulent Activity Notifications<br><a href='https://www.hackerrank.com/challenges/fraudulent-activity-notifications/problem' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>HackerRank</a>",
        category: "Problems - Arrays",
        problem: "<b>Challenge Overview:</b> HackerLand National Bank has a simple policy for warning clients about possible fraudulent account activity. If the amount spent by a client on a particular day is greater than or equal to <code>2 × the client's median spending for a trailing number of days</code>, they send the client a notification about potential fraud.<br><br><b>Input & Constraints:</b> Given the number of trailing days <code>d</code> and a client's total daily expenditures over <code>n</code> days, determine how many notifications will be sent over the entire period.<br>• <code>1 &le; n &le; 200,000</code><br>• <code>1 &le; d &le; n</code><br>• <code>0 &le; expenditure[i] &le; 200</code>",
        solution: "<b>Algorithmic Strategy:</b> We use a <b>Sliding Window Frequency Table (Counting Sort Approach)</b>.<br><br>1. <b>Key Constraint Insight:</b> Each daily expenditure is bounded between <code>0</code> and <code>200</code>. This small, fixed range allows us to track frequencies using a fixed-size array of 201 elements.<br>2. <b>Window Initialization:</b> Populate the frequency table with the first <code>d</code> elements.<br>3. <b>Fast Median Extraction:</b> Iterate through the 201-element frequency table to accumulate counts. If <code>d</code> is odd, find the element at rank <code>d // 2 + 1</code>. If <code>d</code> is even, find the two middle elements at ranks <code>d // 2</code> and <code>d // 2 + 1</code>. Computing <code>2 × median</code> takes at most 201 operations (<code>O(1)</code>).<br>4. <b>Sliding the Window:</b> For each day <code>i</code> from <code>d</code> to <code>n-1</code>, compare <code>expenditure[i]</code> with <code>2 × median</code>. If &ge;, increment notification count. Then update the frequency window in <code>O(1)</code> by decrementing <code>freq[expenditure[i - d]]</code> and incrementing <code>freq[expenditure[i]]</code>.",
        optimality: "<b>Performance Profile:</b><br>• <b>Time Complexity:</b> <code>O(n × MAX_VAL)</code> where <code>MAX_VAL = 201</code>. Across <code>n = 200,000</code>, this takes ~40 million operations, easily completing well within time limits.<br>• <b>Space Complexity:</b> <code>O(MAX_VAL) = O(1)</code> auxiliary space for the 201-bucket frequency array.<br><br><b>Conclusion:</b> A naive sorting approach inside the window would take <code>O(n · d log d)</code>, resulting in Time Limit Exceeded (TLE). The sliding count sort approach is optimal.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def activityNotifications(expenditure, d):\n    freq = [0] * 201\n    notifications = 0\n\n    for x in expenditure[:d]:\n        freq[x] += 1\n\n    def twice_median():\n        count = 0\n        if d % 2:\n            target = d // 2 + 1\n            for val in range(201):\n                count += freq[val]\n                if count >= target:\n                    return val * 2\n        else:\n            target1 = d // 2\n            target2 = target1 + 1\n            first = None\n            for val in range(201):\n                count += freq[val]\n                if first is None and count >= target1:\n                    first = val\n                if count >= target2:\n                    return first + val\n\n    for i in range(d, len(expenditure)):\n        if expenditure[i] >= twice_median():\n            notifications += 1\n        freq[expenditure[i - d]] -= 1\n        freq[expenditure[i]] += 1\n\n    return notifications</pre>",
        stepByStep: `<b>Simulation Audit:</b><br>\n<b>Input:</b> expenditure = [2, 3, 4, 2, 3, 6, 8, 4, 5], d = 5<br><br>\n<b>Initial Window (first 5 days):</b> [2, 3, 4, 2, 3] &rarr; Sorted: [2, 2, 3, 3, 4]<br>\n<div style="padding-left: 20px; border-left: 2px solid #007bff; margin-left: 10px; margin-bottom: 10px;">\n    <i>Day 5 (Amount = 6):</i> Window median = 3, 2 &times; median = 6. 6 &ge; 6 &rarr; <b>Notification sent (Count: 1)</b>.<br>\n    <i>Slide Window:</i> Remove 2, add 6 &rarr; Window: [3, 4, 2, 3, 6].<br>\n    <i>Day 6 (Amount = 8):</i> Window sorted: [2, 3, 3, 4, 6]. Median = 3, 2 &times; median = 6. 8 &ge; 6 &rarr; <b>Notification sent (Count: 2)</b>.<br>\n    <i>Slide Window:</i> Remove 3, add 8 &rarr; Window: [4, 2, 3, 6, 8].<br>\n    <i>Day 7 (Amount = 4):</i> Window sorted: [2, 3, 4, 6, 8]. Median = 4, 2 &times; median = 8. 4 < 8 &rarr; No notification.<br>\n    <i>Slide Window:</i> Remove 4, add 4 &rarr; Window: [2, 3, 6, 8, 4].<br>\n    <i>Day 8 (Amount = 5):</i> Window sorted: [2, 3, 4, 6, 8]. Median = 4, 2 &times; median = 8. 5 < 8 &rarr; No notification.\n</div>\n<b>Final Result:</b> Total notifications = <b>2</b>.`
    },
    {
        id: "matrix-layer-rotation",
        title: "Matrix Layer Rotation<br><a href='https://www.hackerrank.com/challenges/matrix-rotation-algo/problem' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>HackerRank</a>",
        category: "Problems - Arrays",
        problem: "<b>Challenge Overview:</b> You are given a 2D matrix of dimension <code>m × n</code> and a positive integer <code>r</code>. You have to rotate the matrix <code>r</code> times in an anti-clockwise direction.<br><br><b>Constraint & Mechanics:</b> Rotation should be in anti-clockwise direction layer by layer (concentric rings from outer to inner). The minimum of <code>m</code> and <code>n</code> is always even, ensuring every element is part of a complete ring.",
        solution: "<b>Algorithmic Strategy:</b> We peel each concentric layer/ring of the matrix into a 1D array, perform cyclic shift, and write it back.<br><br>1. <b>Layer Extraction:</b> For layer bounded by <code>top, bottom, left, right</code>, traverse elements counter-clockwise: top row (left &rarr; right), right column (top+1 &rarr; bottom-1), bottom row (right &rarr; left), left column (bottom-1 &rarr; top+1).<br>2. <b>Effective Rotation:</b> A ring of length <code>L</code> completes a full cycle every <code>L</code> rotations. Hence, effective rotation is <code>effective_r = r % L</code>.<br>3. <b>Array Slicing / Shifting:</b> Shift the 1D slice counter-clockwise: <code>layer = layer[effective_r:] + layer[:effective_r]</code>.<br>4. <b>Matrix Repopulation:</b> Write the rotated slice elements back into their respective perimeter positions.<br>5. <b>Shrink Boundaries:</b> Increment <code>top</code>, <code>left</code> and decrement <code>bottom</code>, <code>right</code> to process inner layers until boundaries cross.",
        optimality: "<b>Performance Profile:</b><br>• <b>Time Complexity:</b> <code>O(m × n)</code>. Every element in the matrix belongs to exactly one layer and is read and written a constant number of times.<br>• <b>Space Complexity:</b> <code>O(m + n)</code> auxiliary space to hold the elements of the largest perimeter layer during rotation.<br><br><b>Conclusion:</b> This approach is asymptotically optimal since inspecting and updating all <code>m × n</code> matrix cells is unavoidable.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def matrixRotation(matrix, m, n, r):\n    top, bottom, left, right = 0, m - 1, 0, n - 1\n\n    while top < bottom and left < right:\n        # 1. Extract layer elements in counter-clockwise order\n        top_row = matrix[top][left:right + 1]\n        right_col = [matrix[i][right] for i in range(top + 1, bottom)]\n        bottom_row = matrix[bottom][left:right + 1][::-1]\n        left_col = [matrix[i][left] for i in range(bottom - 1, top, -1)]\n\n        layer = top_row + right_col + bottom_row + left_col\n\n        # 2. Rotate 1D layer using modulo arithmetic\n        rot = r % len(layer)\n        layer = layer[rot:] + layer[:rot]\n\n        # 3. Write back into matrix\n        k = 0\n        for j in range(left, right + 1): matrix[top][j] = layer[k]; k += 1\n        for i in range(top + 1, bottom): matrix[i][right] = layer[k]; k += 1\n        for j in range(right, left - 1, -1): matrix[bottom][j] = layer[k]; k += 1\n        for i in range(bottom - 1, top, -1): matrix[i][left] = layer[k]; k += 1\n\n        top += 1; bottom -= 1; left += 1; right -= 1\n    return matrix</pre>",
        stepByStep: `<b>Simulation Audit:</b><br>\n<b>Input Matrix (4x4):</b><br>\n<code>[ 1,  2,  3,  4 ]</code><br>\n<code>[ 5,  6,  7,  8 ]</code><br>\n<code>[ 9, 10, 11, 12 ]</code><br>\n<code>[13, 14, 15, 16 ]</code><br>\n<b>Rotations r:</b> 2<br><br>\n<b>Ring 1 (Outer Layer):</b>\n<div style="padding-left: 20px; border-left: 2px solid #007bff; margin-left: 10px; margin-bottom: 10px;">\n    <i>Extracted CCW:</i> [1, 2, 3, 4, 8, 12, 16, 15, 14, 13, 9, 5] (Length = 12)<br>\n    <i>Rotations:</i> <code>2 % 12 = 2</code><br>\n    <i>Rotated 1D Layer:</i> [3, 4, 8, 12, 16, 15, 14, 13, 9, 5, 1, 2]<br>\n    <i>Written back to outer perimeter.</i>\n</div>\n<b>Ring 2 (Inner Layer):</b>\n<div style="padding-left: 20px; border-left: 2px solid #007bff; margin-left: 10px; margin-bottom: 10px;">\n    <i>Extracted CCW:</i> [6, 7, 11, 10] (Length = 4)<br>\n    <i>Rotations:</i> <code>2 % 4 = 2</code><br>\n    <i>Rotated 1D Layer:</i> [11, 10, 6, 7]<br>\n    <i>Written back to inner ring.</i>\n</div>\n<b>Final Result:</b><br>\n<code>[ 3,  4,  8, 12 ]</code><br>\n<code>[ 2, 11, 10, 16 ]</code><br>\n<code>[ 1,  7,  6, 15 ]</code><br>\n<code>[ 5,  9, 13, 14 ]</code>`
    },
    {
        id: "almost-sorted",
        title: "Almost Sorted<br><a href='https://www.hackerrank.com/challenges/almost-sorted/problem' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>HackerRank</a>",
        category: "Problems - Arrays",
        problem: "<b>Challenge Overview:</b> Given an array of integers, determine whether the array can be sorted in ascending order using only one of the following operations one time: <br>1. Swap two elements.<br>2. Reverse one sub-segment.<br><br><b>Goal:</b> Determine if it's possible and which operation to use.",
        solution: "<b>Algorithmic Strategy:</b> We compare the original array with its sorted version to find mismatches.<br><br>1. Find all indices where the original array differs from the sorted array.<br>2. If there are no mismatches, it's already sorted.<br>3. If we swap the first and last mismatched elements and the array becomes sorted, a <code>swap</code> operation works.<br>4. If we reverse the sub-segment from the first to the last mismatched element and the array becomes sorted, a <code>reverse</code> operation works.<br>5. Otherwise, it cannot be sorted with a single operation.",
        optimality: "<b>Performance Profile:</b><br>• <b>Time Complexity:</b> O(N log N) dominated by sorting the array. Finding mismatches and checking conditions take O(N).<br>• <b>Space Complexity:</b> O(N) to store the sorted array and mismatches.<br><br><b>Conclusion:</b> This approach is straightforward and mathematically robust since we know exactly which elements are out of place.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def almostSorted(arr):\n    sorted_arr = sorted(arr)\n    if arr == sorted_arr: return print(\"yes\")\n\n    mismatches = [i for i in range(len(arr)) if arr[i] != sorted_arr[i]]\n    l, r = mismatches[0], mismatches[-1]\n\n    arr[l], arr[r] = arr[r], arr[l]\n    if arr == sorted_arr: return print(f\"yes\\nswap {l+1} {r+1}\")\n    arr[l], arr[r] = arr[r], arr[l]\n\n    arr[l:r+1] = reversed(arr[l:r+1])\n    if arr == sorted_arr: return print(f\"yes\\nreverse {l+1} {r+1}\")\n\n    print(\"no\")</pre>",
        stepByStep: `<b>Simulation Audit:</b><br>\n<b>Input:</b> [1, 5, 4, 3, 2, 6]<br><br>\n<b>Iteration:</b>\n<div style="padding-left: 20px; border-left: 2px solid #007bff; margin-left: 10px; margin-bottom: 10px;">\n    <i>Sorted:</i> [1, 2, 3, 4, 5, 6]<br>\n    <i>Mismatches at indices:</i> 1, 2, 3, 4 (values 5, 4, 3, 2). l = 1, r = 4.<br>\n    <i>Try swap:</i> [1, 2, 4, 3, 5, 6] -> Not sorted.<br>\n    <i>Try reverse:</i> Reverse [5, 4, 3, 2] -> [2, 3, 4, 5]. Array becomes [1, 2, 3, 4, 5, 6] -> Sorted.\n</div>\n<b>Final Result:</b> yes, reverse 2 5.`
    },
    {
        id: "bomberman-game",
    ...
    },
    {
        ...
        category: "Problems - Arrays",
        problem: "<b>Geometry Objective:</b> Given a 2D board of size H x W where each cell contains a stack of 1x1x1 cubes of height <code>A[i][j]</code>, calculate the total 3D surface area of the resulting geometry. <br><br><b>Constraint:</b> The structure rests on a flat grid. The algorithm must calculate the exposed top, bottom, and side surface areas efficiently.",
        solution: "<b>Geometric Sweep Strategy:</b><br>Rather than calculating the area of each individual cube and subtracting hidden faces, we process the surface area optimally by considering the grid as a whole.<br><br>1. <b>Base Area:</b> The top and bottom surface areas are always <code>2 * H * W</code>, since every non-zero column contributes exactly one top and one bottom face.<br>2. <b>Lateral Area:</b> For each cell, we calculate its exposed side faces by comparing its height to its four immediate neighbors (up, down, left, right).<br>3. <b>Difference Logic:</b> If a cell is taller than its neighbor, the exposed area on that side is exactly <code>height - neighbor_height</code>. If it's shorter or equal, no side area is exposed on that boundary.",
        optimality: "<b>Complexity Analysis:</b><br>• <b>Time:</b> O(H × W) because we perform a single pass over the grid, checking 4 neighbors for each cell.<br>• <b>Space:</b> O(1) auxiliary space, as calculations are done on the fly without extra data structures.<br><br><b>Conclusion:</b> This approach is mathematically optimal, operating strictly in linear time proportional to the number of cells on the board.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def surfaceArea(A):\n    H = len(A)\n    W = len(A[0])\n    area = 2 * H * W\n    for i in range(H):\n        for j in range(W):\n            h = A[i][j]\n            area += max(h - (A[i-1][j] if i > 0 else 0), 0)\n            area += max(h - (A[i+1][j] if i < H-1 else 0), 0)\n            area += max(h - (A[i][j-1] if j > 0 else 0), 0)\n            area += max(h - (A[i][j+1] if j < W-1 else 0), 0)\n    return area</pre>",
        stepByStep: `<b>Geometric Trace:</b><br>\n<b>Input:</b> Board = [[1, 3], [2, 1]]<br><br>\n<b>Computation Phase:</b>\n<div style="padding-left: 20px; border-left: 2px solid #fd7e14; margin-left: 10px; margin-bottom: 10px;">\n    <i>Base Area (Top & Bottom):</i> 2 * 2 * 2 = <b>8</b><br>\n    <i>Cell (0,0) Height=1:</i> Exposed Left=1, Top(neighbor)=1. Exposed Total = 2.<br>\n    <i>Cell (0,1) Height=3:</i> Exposed Right=3, Top=3, Diff with Left(1)=2, Diff with Bot(1)=2. Total = 10.<br>\n    <i>...and so on for other cells.</i>\n</div>\n<b>Final Surface Area:</b> Sum of all exposed faces = <b>22</b>.`
    },
    {
        id: "counting-sort",
        title: "Counting Sort",
...
,
    {
        id: "birthday",
        title: "Birthday<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Count the number of contiguous segments of length `m` in the list `s`<br>whose elements sum to `d`.<br><br>This function checks every possible subarray of length `m` and<br>increments the count when the sum of its elements equals `d`.<br><br>Args:<br>n (int): Total number of elements in the list `s`.<br>s (list[int]): List of integers representing chocolate squares.<br>d (int): Target sum (Ron\u2019s birth day).<br>m (int): Length of the segment (Ron\u2019s birth month).<br><br>Returns:<br>int: Number of valid contiguous segments whose sum equals `d`.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>from logging import exception\n\n\ndef birthday(n: int, s: list[int], d: int, m: int) -&gt; int:\n    \"\"\"\n    Count the number of contiguous segments of length `m` in the list `s`\n    whose elements sum to `d`.\n\n    This function checks every possible subarray of length `m` and\n    increments the count when the sum of its elements equals `d`.\n\n    Args:\n        n (int): Total number of elements in the list `s`.\n        s (list[int]): List of integers representing chocolate squares.\n        d (int): Target sum (Ron\u2019s birth day).\n        m (int): Length of the segment (Ron\u2019s birth month).\n\n    Returns:\n        int: Number of valid contiguous segments whose sum equals `d`.\n    \"\"\"\n    count = 0\n    for i in range(len(s)):\n        if sumArray(s[i:i + m]) == d:\n            count += 1\n    return count\n\n\ndef sumArray(arr: list[int], sum_: int = 0) -&gt; int:\n    \"\"\"\n    Calculate the sum of all integers in a list.\n\n    Args:\n        arr (list[int]): List of integers to sum.\n        sum_ (int, optional): Initial sum value. Defaults to 0.\n\n    Returns:\n        int: Sum of all elements in the list.\n    \"\"\"\n    for i in arr:\n        sum_ += i\n    return sum_\n\n\ndef main() -&gt; None:\n    \"\"\"\n    Read input values, validate them, and print the result of the\n    birthday chocolate calculation.\n\n    Raises:\n        ValueError: If input constraints for `n`, `d`, or `m` are violated.\n\n    Example:\n        &gt;&gt;&gt; birthday(5, [1, 2, 1, 3, 2], 3, 2)\n        2\n    \"\"\"\n    try:\n        n = int(input())\n        s = list(map(int, input().rstrip().split()))\n        d, m = map(int, input().rstrip().split())\n\n        if len(s) != n:\n            raise ValueError(\"The number of values exceeds the allowed limit n\")\n\n        if d &lt; 1 or d &gt; 31:\n            raise ValueError(\"The number of values exceeds the allowed limit d\")\n\n        if m &lt; 1 or m &gt; 12:\n            raise ValueError(\"The number of values exceeds the allowed limit m\")\n\n        print(birthday(n, s, d, m))\n    except Exception as e:\n        return exception(e)\n\n\nif __name__ == '__main__':\n    main()</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "comparelists",
        title: "Compare Lists<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Compares two triplets of ratings and awards points based on category values.<br><br>The comparison logic is as follows:<br>- If list_a[i] > list_b[i], Alice receives 1 point.<br>- If list_a[i] < list_b[i], Bob receives 1 point.<br>- If list_a[i] == list_b[i], neither person receives a point.<br><br>Args:<br>list_a (List[int]): Alice's challenge ratings (clarity, originality, difficulty).<br>list_b (List[int]): Bob's challenge ratings (clarity, originality, difficulty).<br><br>Returns:<br>Tuple[int, int]: A tuple containing (Alice's total score, Bob's total score).<br><br>Example:<br>>>> compareLists([1, 2, 3], [3, 2, 1])<br>(1, 1)<br># Category 1: 1 < 3 (Bob +1)<br># Category 2: 2 == 2 (No points)<br># Category 3: 3 > 1 (Alice +1)",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "equalizearray",
        title: "Equalize Array<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Finds the minimum number of elements to delete to make all elements in the array equal.<br><br>The function identifies the most frequent element in the array and calculates<br>the number of deletions needed by subtracting the frequency of that element<br>from the total number of elements.<br><br>Args:<br>arr (list[int]): An array of integers.<br><br>Returns:<br>int: The minimum number of deletions required.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>\nfrom collections import Counter\n\ndef equalizeArray(arr):\n    \"\"\"\n    Finds the minimum number of elements to delete to make all elements in the array equal.\n    \n    The function identifies the most frequent element in the array and calculates\n    the number of deletions needed by subtracting the frequency of that element \n    from the total number of elements.\n\n    Args:\n        arr (list[int]): An array of integers.\n\n    Returns:\n        int: The minimum number of deletions required.\n    \"\"\"\n    if not arr: return 0\n    counts = Counter(arr)\n    return len(arr) - counts[max(counts, key=counts.get)]\n\ndef main():\n    \"\"\"\n    Main entry point for the script.\n    \n    Reads the array size and elements from standard input, validates the input length,\n    and prints the result of `equalizeArray`.\n    \"\"\"\n    n = int(input().strip())\n    arr = list(map(int, input().rstrip().split()))\n\n    if n != len(arr):\n        raise ValueError(\n            f\"Invalid input: expected {n} elements, got {len(arr)}\"\n        )\n\n    print(equalizeArray(arr))\n\nif __name__ == '__main__':\n    main()</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "happyladybug",
        title: "Happyladybug<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Determines whether all the ladybugs on the board can be made happy.<br>A ladybug is happy if there is at least one other ladybug of the same color adjacent to it.<br>If there is at least one empty space ('_'), we can rearrange the ladybugs arbitrarily.<br><br>Args:<br>b (str): A string representing the board, where '_' is an empty space and<br>uppercase English letters represent colors of ladybugs.<br><br>Returns:<br>str: \"YES\" if all ladybugs can be made happy, otherwise \"NO\".",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>#!/bin/python3\n\nimport math\nimport os\nimport random\nimport re\nimport sys\nfrom collections import Counter\n\ndef happyLadybugs(b):\n    \"\"\"\n    Determines whether all the ladybugs on the board can be made happy.\n    A ladybug is happy if there is at least one other ladybug of the same color adjacent to it.\n    If there is at least one empty space ('_'), we can rearrange the ladybugs arbitrarily.\n    \n    Args:\n        b (str): A string representing the board, where '_' is an empty space and \n                 uppercase English letters represent colors of ladybugs.\n                 \n    Returns:\n        str: \"YES\" if all ladybugs can be made happy, otherwise \"NO\".\n    \"\"\"\n    counts = Counter(b)\n    \n    # If there's a color with exactly 1 ladybug, it can never be happy\n    for char, count in counts.items():\n        if char != '_' and count == 1:\n            return \"NO\"\n            \n    # If there are no empty cells, we must check if they are already happy\n    if counts.get('_', 0) == 0:\n        n = len(b)\n        for i in range(n):\n            if i &gt; 0 and b[i] == b[i-1]:\n                continue\n            if i &lt; n - 1 and b[i] == b[i+1]:\n                continue\n            return \"NO\"\n            \n    return \"YES\"\n\nif __name__ == '__main__':\n    if 'OUTPUT_PATH' in os.environ:\n        fptr = open(os.environ['OUTPUT_PATH'], 'w')\n    else:\n        fptr = sys.stdout\n\n    g = int(input().strip())\n\n    for g_itr in range(g):\n        n = int(input().strip())\n\n        b = input()\n\n        result = happyLadybugs(b)\n\n        fptr.write(result + '\\n')\n\n    if 'OUTPUT_PATH' in os.environ:\n        fptr.close()\n</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "jumpingonclouds",
        title: "Jumping On Clouds<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Calculates the energy level after completing a circular path on clouds.<br><br>Args:<br>c (list[int]): An array of integers representing clouds (0 for normal, 1 for thunderhead).<br>k (int): The jump distance.<br><br>Returns:<br>int: The remaining energy after returning to the starting position.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "jumpingonclouds-0",
        title: "Jumping On Clouds 0<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Calculates the minimum number of jumps required to reach the last cloud.<br><br>The game starts at cloud 0 and the goal is to reach cloud n-1. A player can<br>jump either 1 or 2 clouds away as long as the destination cloud is 'safe'<br>(represented by 0). 'Thunderhead' clouds (represented by 1) must be avoided.<br><br>Args:<br>c (list[int]): A list of binary integers where 0 represents a safe cloud<br>and 1 represents a thunderhead cloud.<br><br>Returns:<br>int: The minimum number of jumps needed to complete the game.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "jumpingoncloudsmin",
        title: "Jumping On Clouds Min<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Calculates the minimum number of jumps required to reach the last cloud.<br><br>The player starts at the first cloud (index 0) and goal is to reach the last<br>cloud (index n-1). The player can jump either 1 or 2 clouds at a time,<br>but must only land on safe clouds (represented by 0).<br><br>Args:<br>c (list[int]): A list of clouds where 0 is safe and 1 is a thunderhead.<br><br>Returns:<br>int: The minimum number of jumps required.<br><br>Example:<br>Input: [0, 0, 1, 0, 0, 1, 0]<br>Output: 4",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "permutationequation",
        title: "Permutation Equation<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Solves the permutation equation by finding the index y such that p(p(y)) == x for each x from 1 to n.<br><br>Args:<br>p (list[int]): A list of n integers where each integer from 1 to n appears exactly once.<br><br>Returns:<br>list[int]: A list of integers representing the values of y for each x from 1 to n.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "strangecode",
        title: "Strange Code<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Finds the value at time t for the strange counter.<br><br>Args:<br>t (int): The time at which to find the counter value.<br><br>Returns:<br>int: The value of the counter at time t.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "thegridsearch",
        title: "The Grid Search<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Finds if a given 2D pattern (p_grid) exists within a larger 2D grid (g_grid).<br>This function uses a rolling hash (Rabin-Karp-like) approach to efficiently<br>find the pattern in the grid.<br><br>Args:<br>g_grid (list of str): The larger grid represented as a list of strings.<br>p_grid (list of str): The smaller pattern grid to search for.<br><br>Returns:<br>str: \"YES\" if the pattern is found, \"NO\" otherwise.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "timeinwords",
        title: "Timeinwords<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Converts a given time (hour and minute) into its words representation.<br><br>For example, 5:00 becomes \"five o' clock\", 5:15 becomes \"quarter past five\",<br>5:30 becomes \"half past five\", and 5:45 becomes \"quarter to six\".<br><br>Args:<br>h (int): The hour (1 <= h <= 12).<br>m (int): The minute (0 <= m < 60).<br><br>Returns:<br>str: A string representing the time in words.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def timeInWords(h, m):\n    \"\"\"\n    Converts a given time (hour and minute) into its words representation.\n\n    For example, 5:00 becomes \"five o' clock\", 5:15 becomes \"quarter past five\",\n    5:30 becomes \"half past five\", and 5:45 becomes \"quarter to six\".\n\n    Args:\n        h (int): The hour (1 &lt;= h &lt;= 12).\n        m (int): The minute (0 &lt;= m &lt; 60).\n\n    Returns:\n        str: A string representing the time in words.\n    \"\"\"\n    words = [\n        \"zero\", \"one\", \"two\", \"three\", \"four\", \"five\", \"six\",\n        \"seven\", \"eight\", \"nine\", \"ten\", \"eleven\", \"twelve\",\n        \"thirteen\", \"fourteen\", \"quarter\", \"sixteen\",\n        \"seventeen\", \"eighteen\", \"nineteen\", \"twenty\",\n        \"twenty one\", \"twenty two\", \"twenty three\", \"twenty four\",\n        \"twenty five\", \"twenty six\", \"twenty seven\", \"twenty eight\",\n        \"twenty nine\"\n    ]\n\n    if m == 0:\n        return f\"{words[h]} o' clock\"\n    elif m == 15:\n        return f\"quarter past {words[h]}\"\n    elif m == 30:\n        return f\"half past {words[h]}\"\n    elif m == 45:\n        return f\"quarter to {words[h + 1 if h &lt; 12 else 1]}\"\n    elif m &lt; 30:\n        minute = \"minute\" if m == 1 else \"minutes\"\n        return f\"{words[m]} {minute} past {words[h]}\"\n    else:\n        rem = 60 - m\n        minute = \"minute\" if rem == 1 else \"minutes\"\n        next_hour = h + 1 if h &lt; 12 else 1\n        return f\"{words[rem]} {minute} to {words[next_hour]}\"\n\nif __name__ == \"__main__\":\n    h, m = map(int, input().split())\n    print(timeInWords(h, m))</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "markscal",
        title: "Marks Cal<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Math",
        problem: "<b>Problem Description:</b><br>Calculate and return the average marks of a student.<br><br>Parameters:<br>m (dict[str, list[float]]): Dictionary mapping student names<br>to a list of their marks.<br>q (str): Name of the student whose average is required.<br><br>Returns:<br>str: The student's average marks formatted to two decimal places.<br><br>Example:<br>>>> m = {\"Alice\": [80, 90, 70], \"Bob\": [60, 75, 85]}<br>>>> marksCal(m, \"Alice\")<br>'80.00'",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def marksCal(m, q):\n    \"\"\"\n    Calculate and return the average marks of a student.\n\n    Parameters:\n        m (dict[str, list[float]]): Dictionary mapping student names\n            to a list of their marks.\n        q (str): Name of the student whose average is required.\n\n    Returns:\n        str: The student's average marks formatted to two decimal places.\n\n    Example:\n        &gt;&gt;&gt; m = {\"Alice\": [80, 90, 70], \"Bob\": [60, 75, 85]}\n        &gt;&gt;&gt; marksCal(m, \"Alice\")\n        '80.00'\n    \"\"\"\n    return f\"{sum_(m[q]) / 3:.2f}\"\n\n\ndef sum_(ar: list[int]) -&gt; float:\n    \"\"\"\n    Compute the sum of a list of numbers.\n\n    Parameters:\n        ar (list[int]): List of numeric values.\n\n    Returns:\n        float: Sum of the elements in the list.\n\n    Example:\n        &gt;&gt;&gt; sum_([1, 2, 3])\n        6\n    \"\"\"\n    return sum(ar)\n\n\ndef main():\n    \"\"\"\n    Read student data, process marks, and print the average\n    marks for a queried student.\n\n    Input format:\n        - An integer n representing the number of students.\n        - n lines each containing a student name followed by 3 marks.\n        - A final line containing the queried student name.\n\n    Output:\n        - The average marks of the queried student formatted to two decimals.\n\n    Sample Input:\n        2\n        Alice 80 90 70\n        Bob 60 75 85\n        Alice\n\n    Sample Output:\n        80.00\n    \"\"\"\n    n = int(input())\n    m = {name: list(map(float, scores))\n                 for name, *scores in (input().split() for _ in range(n))}\n    q = input()\n    print(marksCal(m, q))\n\n\nif __name__ == \"__main__\":\n    main()\n</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "squares",
        title: "Squares<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Math",
        problem: "<b>Problem Description:</b><br>Counts the number of square integers between two given integers a and b (inclusive).<br><br>The number of squares in [a, b] is found by calculating the number of squares<br>up to b (int(sqrt(b))) and subtracting the number of squares up to a-1 (int(sqrt(a-1))).<br><br>Args:<br>a (int): The starting integer of the range.<br>b (int): The ending integer of the range.<br><br>Returns:<br>int: Total count of square integers in the range [a, b].",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "countapplesandoranges",
        title: "Count Apples And Oranges<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Counts how many apples and oranges land on a house.<br><br>The house is located between points `s` and `t` (inclusive).<br>Apples fall from point `a`, oranges fall from point `b`.<br>Each value in `ac` and `oc` represents the distance a fruit<br>falls from its tree.<br><br>Args:<br>s (int): Start position of the house.<br>t (int): End position of the house.<br>a (int): Position of the apple tree.<br>b (int): Position of the orange tree.<br>ac (List[int]): Distances each apple falls from the apple tree.<br>oc (List[int]): Distances each orange falls from the orange tree.<br><br>Returns:<br>List[int]: A list containing:<br>- number of apples that land on the house<br>- number of oranges that land on the house",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "getmoneyspent",
        title: "Get Money Spent<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Find the maximum amount of money that can be spent on one keyboard and one drive<br>without exceeding the budget using a brute-force approach.<br><br>Parameters:<br>k (list of int): Prices of available keyboards.<br>d (list of int): Prices of available USB drives.<br>b (int): Budget constraint.<br><br>Returns:<br>int: Maximum sum of one keyboard and one drive that does not exceed the budget.<br><br>Notes:<br>- Time complexity: O(n * m), where n = len(k) and m = len(d).<br>- May be inefficient for large input sizes due to nested loops.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "hurdlerace",
        title: "Hurdle Race<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Calculates the minimum doses of potion needed to jump over all hurdles.<br><br>Each dose of potion increases the maximum height the character can jump by 1 unit.<br><br>Args:<br>k (int): The maximum height the character can jump without potion.<br>height (list[int]): A list of hurdle heights.<br><br>Returns:<br>int: The minimum number of doses needed.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "queensattack",
        title: "Queens Attack<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Calculates the total number of squares the queen can attack on an n x n board.<br><br>The function considers 8 directions (horizontal, vertical, and diagonal) and<br>accounts for any obstacles that might block the queen's path.<br><br>Args:<br>board_size (int): The number of rows and columns on the board.<br>obstacle_count (int): The number of obstacles on the board.<br>queen_row (int): The row number of the queen's position.<br>queen_column (int): The column number of the queen's position.<br>obstacles (list[list[int]]): A list of obstacle positions, where each<br>obstacle is represented as [row, column].<br><br>Returns:<br>int: The total number of squares the queen can attack.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "stones",
        title: "Stones<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Calculates all possible values of the last stone after n stones.<br><br>Manasa reaches the n-th stone by taking n-1 steps. Each step<br>can be of length 'a' or 'b'. This function finds all unique<br>possible values of the final stone.<br><br>Args:<br>n (int): Total number of stones.<br>a (int): Possible step length 1.<br>b (int): Possible step length 2.<br><br>Returns:<br>list: Sorted list of unique possible values for the n-th stone.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>import sys\n\ndef stones(n, a, b):\n    \"\"\"\n    Calculates all possible values of the last stone after n stones.\n    \n    Manasa reaches the n-th stone by taking n-1 steps. Each step\n    can be of length 'a' or 'b'. This function finds all unique\n    possible values of the final stone.\n    \n    Args:\n        n (int): Total number of stones.\n        a (int): Possible step length 1.\n        b (int): Possible step length 2.\n        \n    Returns:\n        list: Sorted list of unique possible values for the n-th stone.\n    \"\"\"\n    res = {(i * b + (n - 1 - i) * a) for i in range(n)}\n    return sorted(res)\n\ndef main():\n    \"\"\"\n    Main execution block to handle input/output for HackerRank.\n    Reads the number of test cases and parameters for each case.\n    \"\"\"\n    it = iter(map(int, sys.stdin.read().split()))\n    try:\n        t = next(it)\n        for _ in range(t):\n            n, a, b = next(it), next(it), next(it)\n            print(*stones(n, a, b))\n    except StopIteration:\n        pass\n\nif __name__ == \"__main__\":\n    main()</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Execution trace requires manual dry-run of the code blocks.`
    },
    {
        id: "workbook",
        title: "Workbook<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Simulation",
        problem: "<b>Problem Description:</b><br>Calculates the number of special problems in Lisa's workbook.<br>A problem is considered \"special\" if its index (within a chapter)<br>is the same as the page number it's located on.<br><br>Args:<br>n (int): The number of chapters.<br>k (int): The maximum number of problems per page.<br>arr (list[int]): An array where arr[i] denotes the number of problems in the (i+1)-th chapter.<br><br>Returns:<br>int: The total number of special problems in the workbook.",
        solution: "<b>Approach:</b><br>The algorithm leverages logical patterns and data structures to arrive at the optimal solution efficiently. The specific logic is mapped out in the implementation below.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> Optimal complexity.<br>• <b>Space:</b> O(1) or O(N) depending on data structures used.",
    ...
    {
        id: "larrys-array",
        title: "Larry's Array<br><a href='https://github.com/AnamolZ/algovault' target='_blank' style='font-size: 0.9rem; color: #007bff; text-decoration: none;'>View Source</a>",
        category: "Problems - Arrays",
        problem: "<b>Problem Description:</b><br>Determine whether an array can be sorted by rotating any 3 consecutive elements.<br><br>Args:<br>A (list[int]): An array of integers.<br><br>Returns:<br>string: \"YES\" if the array can be sorted, otherwise \"NO\".",
        solution: "<b>Approach:</b><br>The operation of rotating 3 consecutive elements preserves the parity of the total number of inversions in the array. Thus, the array can be sorted if and only if the initial number of inversions is even.",
        optimality: "<b>Complexity:</b><br>• <b>Time:</b> O(N^2) to count inversions (can be optimized to O(N log N) using Merge Sort or Fenwick tree, but O(N^2) is acceptable for small N).<br>• <b>Space:</b> O(1) auxiliary space.",
        codeBlock: "<pre style='background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; overflow-x: auto; margin-top: 10px; font-family: Fira Code, monospace; font-size: 0.95rem; border: 1px solid #333;'>def larrysArray(A):\n    \"\"\"\n    Determines if the array can be sorted by rotating any 3 consecutive elements.\n    This operation preserves the parity of the number of inversions.\n    Returns 'YES' if inversions count is even, else 'NO'.\n    \"\"\"\n    inversions = 0\n    n = len(A)\n    \n    for i in range(n):\n        for j in range(i + 1, n):\n            if A[i] > A[j]:\n                inversions += 1\n\n    return \"YES\" if inversions % 2 == 0 else \"NO\"</pre>",
        stepByStep: `<b>Algorithm Trace:</b><br>Count the number of inversions (pairs i &lt; j where A[i] &gt; A[j]). If the count is even, return YES, otherwise return NO.`
    }
];
